...
import {
  actionButton, badge, chipErr, chipWarn, EmptyState, gateCard, panelBody, panelHead, panelStyle,
} from './styles.ts'
...
const closeBtn: React.CSSProperties = {
  ...
  cursor: 'pointer', fontSize: 14, borderRadius: 6, padding: '2px 6px', lineHeight: 1,
}

const collapsibleStyle: React.CSSProperties = {
  borderTop: '1px solid var(--dsw-alias-border-l1)', marginTop: 6, paddingTop: 8,
}

const inboxRow: React.CSSProperties = {
  fontSize: 12, color: 'var(--dsw-alias-label-secondary)', marginTop: 5, display: 'flex', gap: 6,
  overflowWrap: 'anywhere',
}

/** Floating collaboration panel: header + peers + pending-gate cards + collapsible inbox. */
    ...
    <>
        ...
        <div style={modalStyle}>
          <div style={modalCard}>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 8, justifyContent: 'flex-end' }}>
              ...
              <button type="button" className="p2p-btn" style={{ ...actionButton, flex: 'none', background: 'var(--dsw-alias-brand-primary)', color: '#fff' }} onClick={submitEdit}>{t('overlay.send')}</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {!open ? null : (
        <div style={panelStyle}>
          <div style={panelHead}>
            <span style={{ fontWeight: 700 }}>{t('overlay.title')}</span>
            {gateCount > 0 ? <span style={badge}>{gateCount}</span> : null}
            <span style={{ flex: 1 }} />
          ...
          {!hasContent ? (
            <div style={panelBody}>
              <EmptyState icon="👥" text={t('overlay.noPeers')} />
            </div>
          ) : (
            <div style={panelBody}>
              {peerList.length > 0 ? (
              ...
              {gates.map(gate => {
                ...
                return (
                  <div key={gate.id} style={gateCard}>
                    ...
                    <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                      <button type="button" className="p2p-btn" style={actionButton} disabled={draftEmpty} onClick={() => { void approveGate(gate.id).then((ok) => { if (!ok) setNotice(t('overlay.emptyReplyNotice')); refresh() }) }}>{t('overlay.approve')}</button>
                      <button type="button" className="p2p-btn" style={actionButton} onClick={() => startEdit(gate)}>{t('overlay.edit')}</button>
                      <button type="button" className="p2p-btn" style={actionButton} onClick={() => { void rejectGate(gate.id).then(refresh) }}>{t('overlay.reject')}</button>
                    </div>
                  </div>
                )
              })}
              ...
              {inbox.length > 0 ? (
                <details className="p2p-details" style={collapsibleStyle} open>
