        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 8, justifyContent: 'flex-end' }}>
              <button type="button" style={{ ...actionButton, flex: 'none' }} onClick={() => setEditing(null)}>{t('overlay.cancel')}</button>
              <button type="button" style={{ ...actionButton, flex: 'none', background: 'var(--dsw-alias-brand-primary)', color: 'var(--dsw-alias-label-primary-inverted)' }} onClick={submitEdit}>{t('overlay.send')}</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {!open ? null : (
        <div style={{
          ...
          <h3 style={{ margin: 0, fontSize: 14 }}>
            {gates.length > 0 ? t('overlay.titleGates', { count: gates.length }) : t('overlay.title')}
          </h3>
          {peerList.length > 0 ? (
            ...
            <div style={{ fontSize: 12, color: 'var(--dsw-alias-label-tertiary)', marginTop: 4 }}>{t('overlay.noPeers')}</div>
          )}
          {inbox.length > 0 ? (
            <div style={{ borderTop: '1px solid var(--dsw-alias-border-l1)', marginTop: 8, paddingTop: 8 }}>
              <div style={{ fontWeight: 600, fontSize: 12 }}>{t('overlay.inbox')}</div>
              {inbox.map(message => (
              ...
              <div key={gate.id} style={{ borderTop: '1px solid var(--dsw-alias-border-l1)', paddingTop: 8, marginTop: 8 }}>
                ...
                {draftEmpty
                  ...
                  : <div style={{ marginTop: 4, fontSize: 12 }}>
                    <span style={{ fontWeight: 600 }}>{t('overlay.aiDraft')}</span>{gate.draftBody}
                  </div>}
                <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                  <button type="button" style={actionButton} disabled={draftEmpty} onClick={() => { void approveGate(gate.id).then((ok) => { if (!ok) setNotice(t('overlay.emptyReplyNotice')); refresh() }) }}>{t('overlay.approve')}</button>
                  <button type="button" style={actionButton} onClick={() => startEdit(gate)}>{t('overlay.edit')}</button>
                  <button type="button" style={actionButton} onClick={() => { void rejectGate(gate.id).then(refresh) }}>{t('overlay.reject')}</button>
                </div>
              </div>
            )
          })}
